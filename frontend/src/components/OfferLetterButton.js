...
const OfferLetterButton = ({ studentData }) => {
    const handleGenerateOfferLetter = async () => {
        try {
            ...
            const fileName = studentId.substring(studentId.length - 3).replace(/^0+/, ''); // Remove leading zeroes
            
            // Send request to server to generate offer letter
            const response = await axios.post('/api/pdf/generate', studentData, {
    ...
    return (
        <button className="btn btn-outline-primary" onClick={handleGenerateOfferLetter}>
            Offer Letter
        </button>
    );
};
